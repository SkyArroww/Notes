...
- 通配符 - 当你想要利用通配符进行匹配时，你可以**分别使用 `?` 和 `*` 来匹配一个或任意个字符**。例如，对于文件`foo`, `foo1`, `foo2`, `foo10` 和 `bar`, `rm foo?`这条命令会删除`foo1` 和 `foo2` ，而`rm foo*` 则会删除除了`bar`之外的所有文件。
...
### 2.2.2 查找文件
* 使用`find`进行文件查找，可以指定查询路径`-path '****'`，指定文件类型`-type '****'`，文件大小、文件名等等
* 使用`locate`查找文件系统中包含想要的子字符串的路径，会保存索引，使用`updatedb`更新索引
