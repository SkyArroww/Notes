...
* cd ~: 返回用户主目录
* cd -: 返回刚才所在目录
除非我们利用第一个参数指定目录，否则 `ls` 会打印当前目录下的文件。大多数的命令接受标记和选项（带有值的标记），它们以 `-` 开头，并可以改变程序的行为。通常，在执行程序时使用 `-h` 或 `--help` 标记可以打印更详细的帮助信息，以便了解有哪些可用的标记或选项。例如，`ls --help` 的输出如下：
```powershell
  -* ****
...
在这个阶段，还有几个趁手的命令是您需要掌握的，例如 `mv`（用于重命名或移动文件）、 `cp`（拷贝文件）以及 `mkdir`（新建文件夹）`Ctrl+l` (清空并返回顶部)
...
hello
```
将 `hello.txt` 的输出重定向给 `cat`, 再将 `cat` 输出到 `hello2.txt` 中

您还可以使用 `>>` 来向一个文件追加内容。使用管道（ _pipes_ ），我们能够更好的利用文件重定向。 `|` 操作符允许我们**将一个程序的输出和另外一个程序的输入连接起来**：
```powershell
missing:~$ ls -l / | tail -n1
drwxr-xr-x 1 root  root  4096 Jun 20  2019 var
missing:~$ curl --head --silent google.com | grep -i content-length | cut --delimiter=' ' -f2
219
```
e.g. 将 `ls` 的输出作为 `tail` 的输入
...
因为打开 `/sys` 文件的是 `tee` (同时将输入写入文件与打印)这个程序，并且该程序以 `root` 权限在运行，因此操作可以进行。 这样您就可以在 `/sys` 中愉快地玩耍了，例如修改系统中各种LED的状态（路径可能会有所不同）：

```powershell
$ echo 1 | sudo tee /sys/class/leds/input6::scrolllock/brightness
```

同样地，我们也可以先将身份切换成 `root` 再运行以上命令， 此时 `$` 提示符会变为`#`
